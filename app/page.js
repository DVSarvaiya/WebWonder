...
export default function Home() {
  return (
    <main className="relative w-full h-screen bg-[#000000] text-white font-sans">
      ...
      {/* Border Frame */}
      <div className="fixed top-5 bottom-5 left-5 right-5 border-2 border-cyan-500 z-10 rounded-xl pointer-events-none"></div>


      {/* Navigation */}
      <div className="absolute top-10 right-10 z-20 flex space-x-6 text-sm uppercase tracking-widest font-medium">
        {['Home', 'Creatures', 'Explore', 'Technologies', 'About Us'].map((label, i) => (
          <a
            key={i}
            href="#"
            className="text-white hover:text-white transition-colors duration-300 relative after:absolute after:bottom-0 after:left-0 after:w-0 after:h-[2px] after:bg-cyan-400 after:transition-all after:duration-300 hover:after:w-full"
          >
            {label}
          </a>
        ))}
